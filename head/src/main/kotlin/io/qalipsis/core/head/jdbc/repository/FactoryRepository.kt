...
internal interface FactoryRepository : CoroutineCrudRepository<FactoryEntity, Long> {
    ...
    @Query(
        """SELECT factory.*, 
            ...
            AND EXISTS (SELECT * FROM tenant WHERE reference = :reference AND id = factory.tenant_id)
            """
    )
    @Join(value = "selectors", type = Join.Type.LEFT_FETCH)
    suspend fun findByNodeIdIn(factoryIds: Collection<String>, reference: String): List<FactoryEntity>
    ...
    suspend fun findIdByNodeIdIn(factoryIds: Collection<String>, reference: String): List<Long>
    ...
    @Query(
        """SELECT factory.*, 
            ...
            AND EXISTS (SELECT * FROM tenant WHERE reference IN (:reference) AND id = factory.tenant_id)
                """
    )
    @Join(value = "selectors", type = Join.Type.LEFT_FETCH)
    suspend fun getAvailableFactoriesForScenarios(names: Collection<String>, reference: Collection<String>): List<FactoryEntity>
