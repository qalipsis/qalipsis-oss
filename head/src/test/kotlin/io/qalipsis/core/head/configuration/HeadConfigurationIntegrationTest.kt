...
internal class HeadConfigurationIntegrationTest {
    ...
    @Timeout(10)
    fun `should create the head configuration with the default values`(headConfiguration: HeadConfiguration) {
        assertThat(headConfiguration).all {
            ...
            prop(HeadConfiguration::cluster).all {
                ...
                prop(ClusterConfiguration::zones).isEmpty()
            }
        }
    }
    ...
    @MicronautTest(startApplication = false)
    @Timeout(10)
    fun `should create the head configuration with specified values`(headConfiguration: HeadConfiguration) {
        assertThat(headConfiguration).all {
            ...
            prop(HeadConfiguration::cluster).all {
                ...
                prop(ClusterConfiguration::zones).all {
                    hasSize(2)
                    containsAll(
                        Zone(
                            key = "fr",
