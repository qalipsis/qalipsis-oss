...
internal class WidgetServiceImplTest {
    ...
    internal fun `should retrieve latest factory state for each factory id per tenant`() =
        testDispatcherProvider.runTest {
            ...
            coEvery {
                factoryStateRepository.findLatestFactoryStates(any())
            ...
            coVerifyOnce {
                factoryStateRepository.findLatestFactoryStates("my-tenant")
            }
        }
    ...
    internal fun `should retrieve the campaign results and their states when start and end is provided`() =
        testDispatcherProvider.runTest {
            ...
            coEvery {
                campaignRepository.aggregate(any(), any(), any(), any())
    ...
    internal fun `should retrieve the campaign results and their states using default values`() =
        testDispatcherProvider.runTest {
            ...
            coVerifyOnce {
                campaignRepository.aggregate(
                    "my-tenant",
                    startIdentifier = Instant.now().truncatedTo(ChronoUnit.DAYS).minus(calcOffset),
                    endIdentifier = Instant.now().truncatedTo(ChronoUnit.DAYS).minus(calcOffset)
        ...
        testDispatcherProvider.runTest {
            ...
            coVerifyOnce {
                campaignRepository.aggregate(
                    "my-tenant",
                    startIdentifier = now.minus(offset.toLong(), ChronoUnit.HOURS),
                    endIdentifier = now.minus(offset.toLong(), ChronoUnit.HOURS).minus(7, ChronoUnit.DAYS),
                    timeframe = timeFrame
                )
            }
        }
