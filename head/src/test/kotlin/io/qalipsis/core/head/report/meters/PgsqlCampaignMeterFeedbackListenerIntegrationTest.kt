...
import io.qalipsis.core.configuration.ExecutionEnvironments.POSTGRESQL
...
@Testcontainers
@MicronautTest(environments = [POSTGRESQL], startApplication = false, transactional = false)
internal class PgsqlCampaignMeterFeedbackListenerIntegrationTest :
    AbstractCampaignMeterFeedbackListenerTest(), TestPropertyProvider {

    @Inject
    @field:Named("default")
    private lateinit var jdbcDataSource: DataSource

    override val verificationDataSource: DataSource
        get() = DelegatingDataSource.unwrapDataSource(jdbcDataSource)
