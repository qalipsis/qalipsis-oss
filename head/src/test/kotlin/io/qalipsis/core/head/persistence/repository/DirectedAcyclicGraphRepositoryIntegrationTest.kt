...
import io.qalipsis.core.head.persistence.repository.PostgresqlTemplateTest
...
internal class DirectedAcyclicGraphRepositoryIntegrationTest : PostgresqlTemplateTest() {
    ...
    @BeforeAll
    internal fun setUpAll(factoryRepository: FactoryRepository, scenarioRepository: ScenarioRepository): Unit =
        runBlocking {
            val factory = factoryRepository.save(
                FactoryEntity(nodeId = "the-node", registrationTimestamp = Instant.now(), registrationNodeId = "test")
            )
