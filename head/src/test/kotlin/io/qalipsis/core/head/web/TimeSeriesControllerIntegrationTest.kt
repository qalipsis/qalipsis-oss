...
internal class TimeSeriesControllerIntegrationTest {
    ...
    @Test
    internal fun `should retrieve the campaign summary when end and timeframe is not passed in`() {
        // given
        val timeOffset = 0.30F
        val start = Instant.parse("2022-10-29T00:00:00.00Z")
        val startInterval = start.minus(30, ChronoUnit.MINUTES)
        val timeframe = Duration.of(1, ChronoUnit.HOURS).minusMinutes(20)
        val campaignSummaryList = listOf(
        ...
        val request =
            HttpRequest.GET<List<CampaignSummaryResult>>("/time-series/summary/campaign-status?from=$start&timeOffset=$timeOffset")
        coEvery {
            widgetService.aggregateCampaignResult(any(), any(), any(), any(), any())
        } returns campaignSummaryList
