...
internal abstract class AbstractTopicSubscription<T>(
    ...
    protected suspend fun init(idleCoroutineContext: CoroutineContext) {
        if (idleTimeout.toMillis() > 0) {
            pollNotificationChannel = Channel(1)
            timeout = buildTimeout()
            idleVerificationJob = withContext(idleCoroutineContext) {
                launch {
                    try {
    ...
    override suspend fun onReceiveValue(context: CoroutineContext, block: suspend (T) -> Unit): Job {
        verifyState()
        receivingJob?.cancelAndJoin()
        receivingJob = withContext(context) {
            launch {
