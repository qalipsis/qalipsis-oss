...
onBeforeUnmount(() => campaignsTableStore.$reset())
...
const handleCreateReportBtnClick = async () => {
  if (selectedRowKeys.value.length === 0 && campaignPatterns.value.length === 0) {
    toastStore.error({ text: 'Select at least one campaign or enter a campaign pattern.' })
    return
  }
