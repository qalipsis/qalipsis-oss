<template>
  <section
    v-if="open"
    class="px-1 py-2 rounded-md border border-solid border-gray-300 bg-white shadow-md"
  >
    <header class="flex justify-between">
      ...
      >
        <BaseIcon
          icon="qls-icon-close"
          class="text-base hover:text-primary-500"
        ></BaseIcon>
      </div>
    </header>
    <Sketch
      :model-value="hexCodeValue"
      @update:model-value="handleColorChange($event)"
    />
  </section>
</template>
