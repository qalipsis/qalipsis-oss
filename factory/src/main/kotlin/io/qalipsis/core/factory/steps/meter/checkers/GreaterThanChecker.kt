...
/**
 * Asserts the value is greater than the expected threshold, using the "greater than" operator.
 *
 *  @param threshold the reference value used in comparison
 *
 * @author Francisca Eze
 */
class GreaterThanChecker<T : Comparable<T>>(private val threshold: T) : ValueChecker<T> {

    override fun check(value: T): MeterAssertionViolation? {
        return supplyUnless(value > threshold) {
            MeterAssertionViolation("Value should be greater than $threshold")
        }
    }
}