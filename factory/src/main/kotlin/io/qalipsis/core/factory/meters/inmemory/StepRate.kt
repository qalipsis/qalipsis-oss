...
internal class StepRate(
    ...
    override suspend fun snapshot(timestamp: Instant): MeterSnapshot =
        MeterSnapshotImpl(timestamp, id.copy(tags = id.tags + ("scope" to "period")), this.measure())

    override suspend fun summarize(timestamp: Instant): MeterSnapshot = snapshot(timestamp)
