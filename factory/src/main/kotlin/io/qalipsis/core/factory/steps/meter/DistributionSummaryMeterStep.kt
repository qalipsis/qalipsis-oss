...
/**
 * Step to create a [DistributionSummary] that allows for specification of failure conditions
 * and evaluation of success and failure of the steps using configured thresholds.
 *
 * @author Francisca Eze
 */
class DistributionSummaryMeterStep<I>(
    id: StepName,
    ...
    private val checkers: List<Pair<DistributionSummary.() -> Double, ValueChecker<Double>>>,
    private val campaignMeterRegistry: CampaignMeterRegistry,
    private val checkPeriod: Duration = Duration.ofMillis(1000)
) : AbstractStep<I, I>(id, retryPolicy) {

    private lateinit var meter: DistributionSummary
    ...
    override suspend fun start(context: StepStartStopContext) {
        if (initialized.compareAndSet(false, true)) {
            meter = campaignMeterRegistry.summary(
