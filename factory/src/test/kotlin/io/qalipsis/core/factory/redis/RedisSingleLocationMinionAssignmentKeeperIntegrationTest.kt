...
internal class RedisSingleLocationMinionAssignmentKeeperIntegrationTest : AbstractRedisIntegrationTest() {
    ...
    @MockBean(CommunicationChannelConfiguration::class)
    fun communicationChannelConfiguration() = communicationChannelConfiguration
    ...
    internal fun `should assign a lot of minions only for the DAGs supported by the factory`(minionAssignmentKeeper: RedisSingleLocationMinionAssignmentKeeper) =
        testDispatcherProvider.run {
            ...
            val assignedFactory1Scenario1 = assignedFactory1Scenario1Job.await().toMutableMap()
            val assignedFactory2Scenario1 = assignedFactory2Scenario1Job.await().toMutableMap()
            ...
            // The singletons minions are assigned to both factories.
            assertThat(assignedFactory1Scenario1).all {
                key(MINIONS_SINGLETON_1).isNotNull().containsOnly(SCENARIO_1_DAG_SINGLETON_1)
                key(MINIONS_SINGLETON_2).isNotNull().containsOnly(SCENARIO_1_DAG_SINGLETON_2)
            }
            assertThat(assignedFactory2Scenario1).all {
                key(MINIONS_SINGLETON_1).isNotNull().containsOnly(SCENARIO_1_DAG_SINGLETON_1)
                key(MINIONS_SINGLETON_2).isNotNull().containsOnly(SCENARIO_1_DAG_SINGLETON_2)
            }
            verifyOnce {
                localAssignmentStore.save(SCENARIO_1, eq(assignedFactory1Scenario1))
                localAssignmentStore.save(SCENARIO_1, eq(assignedFactory2Scenario1))
                localAssignmentStore.save(SCENARIO_2, eq(assignedFactory1Scenario2))
                localAssignmentStore.save(SCENARIO_2, eq(assignedFactory2Scenario2))
            }

            assignedFactory1Scenario1.remove(MINIONS_SINGLETON_1)
            assignedFactory1Scenario1.remove(MINIONS_SINGLETON_2)
            assignedFactory2Scenario1.remove(MINIONS_SINGLETON_1)
            assignedFactory2Scenario1.remove(MINIONS_SINGLETON_2)

            // Verifies that all the DAGs are assigned only once for the minions under load.
            ...
            // Verifies the assignments are complete for the non singletons.
            ...
            // The factory should always return itself as channel of the requested assignments.
            every { communicationChannelConfiguration.unicastChannel } returns "the-random-channel"
            val channelForAllMinions = minionAssignmentKeeper.getFactoriesChannels(
                CAMPAIGN,
                SCENARIO_1,
            ...
            ).rowMap().flatMap { it.value.map { it.value } }.toSet()

            // then
            assertThat(channelForAllMinions).containsOnly("the-random-channel")
        }
    ...
    @Timeout(10)
    @Order(3)
    internal fun `should schedule the minions underload of all factories`(minionAssignmentKeeper: RedisSingleLocationMinionAssignmentKeeper) =
        testDispatcherProvider.run {
            // when
            minionAssignmentKeeper.schedule(
                CAMPAIGN, SCENARIO_1, listOf(
                    ...
                    MinionsStartingLine(MINIONS_COUNT_IN_EACH_SCENARIO, 789),
                )
            )
            minionAssignmentKeeper.schedule(
                CAMPAIGN, SCENARIO_2, listOf(
                    ...
                    MinionsStartingLine(MINIONS_COUNT_IN_EACH_SCENARIO, 2789),
                )
            )
            ...
            minionAssignmentKeeper.readSchedulePlan(CAMPAIGN, SCENARIO_2, "the-factory-1-channel")
                .forEach { (offset, minions) ->
                    scheduleForScenario2.computeIfAbsent(offset) { mutableSetOf() } += minions
                }
            minionAssignmentKeeper.readSchedulePlan(CAMPAIGN, SCENARIO_2, "the-factory-2-channel")
                .forEach { (offset, minions) ->
                    scheduleForScenario2.computeIfAbsent(offset) { mutableSetOf() } += minions
                }
    ...
    internal fun `should assign until the timeout`(minionAssignmentKeeper: RedisSingleLocationMinionAssignmentKeeper) =
        testDispatcherProvider.run {
            ...
            // when
            org.junit.jupiter.api.assertThrows<TimeoutCancellationException> {
    ...
    internal fun `should assign allowed count of minions`(minionAssignmentKeeper: RedisSingleLocationMinionAssignmentKeeper) =
        testDispatcherProvider.run {
            // given
            ...
            assertThat(assignedMinions).all {
                hasSize(321 + 50) // All the minions, including the singletons.
