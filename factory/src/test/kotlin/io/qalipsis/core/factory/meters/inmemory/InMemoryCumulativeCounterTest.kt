...
internal class InMemoryCumulativeCounterTest {
    ...
    internal fun `should increment the counter by 1 when increment is called without an argument`() {
        ...
        val counter = InMemoryCumulativeCounter(id, meterReporter)
    ...
    internal fun `should build the snapshot and keep on cumulating`() = testDispatcherProvider.run {
        ...
        assertThat(result).isInstanceOf(MeterSnapshotImpl::class).all {
            ...
            prop(MeterSnapshotImpl::measurements).transform { it.toList() }.all {
                hasSize(1)
                index(0).all {
                    prop(Measurement::value).isEqualTo(3.0)
                    prop(Measurement::statistic).isEqualTo(Statistic.COUNT)
                }
            }
        }
    ...
    internal fun `should build the total snapshot`() = testDispatcherProvider.run {
        ...
        //when
        var result = counter.summarize(now)
