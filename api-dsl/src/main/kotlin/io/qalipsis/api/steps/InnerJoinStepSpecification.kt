...
/**
 * Variant of [innerJoin] to use for auto-completion in IDEs. Once the scenario compile with this,
 * replace the operator by [innerJoin] to have a strongly types output.
 *
 * @author Eric Jessé
 */
fun <INPUT> StepSpecification<*, INPUT, *>.innerJoinUncasted(
    using: (CorrelationRecord<INPUT>) -> Any?,
    on: (scenario: ScenarioSpecification) -> StepSpecification<*, Any?, *>,
    having: (CorrelationRecord<*>) -> Any?
) = this.innerJoin(using = using, on = on, having = having)
