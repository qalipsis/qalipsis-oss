...
const tagSchema = {
    ...
    operator: toTypedSchema(zod.string().nonempty(requiredErrorMessage).nullable()),
 ...
 */
const tagNameOptions = computed<FormMenuOption[]>(() => Object.keys(props.tagMap).map(t => ({
    label: t,
    value: t
})))
...
const _prepareTagValueOptions = (tagName: string) => {
    ...
    tagValueOptions.value = tagValues ? tagValues.map(tv => ({
        label: tv,
        value: tv
    })): [];
}
