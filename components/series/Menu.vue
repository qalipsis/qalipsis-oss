<template>
  <section>
    ...
    <div class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-2">
      <template
        v-for="dataSeriesOption in availableDataSeriesOptions"
...
const { getAllCachedDataSeries, fetchDataSeries } = useDataSeriesApi()
...
onMounted(async () => {
  try {
    ...
    const allDataSeries = (await getAllCachedDataSeries()).filter(
    ...
    availableDataSeriesOptions.value = dataSeriesOptions.value
      .slice(0, SeriesDetailsConfig.MAX_DATA_SERIES_TO_BE_DISPLAYED)
...
const handleSearch = async (searchTerm: string) => {
  // FIXME: to confirm if FE should do the fuzzy search from all data series.
  // const searchKeys = ['reference', 'displayName', 'dataType', 'valueName']
  // const searchedResult = SearchHelper.performFuzzySearch<DataSeriesOption>(
  //   searchTerm,
  //   dataSeriesOptions.value,
  //   searchKeys
  // )

  const request: PageQueryParams = {
    page: 0,
    size: 100,
  }

  if (searchTerm) {
    request.filter = TableHelper.getSanitizedQuery(searchTerm)
  }

  const searchedResult = await fetchDataSeries(request)

  availableDataSeriesOptions.value = SeriesHelper.toDataSeriesOptions(
    searchedResult.elements.slice(0, SeriesDetailsConfig.MAX_DATA_SERIES_TO_BE_DISPLAYED + 1),
