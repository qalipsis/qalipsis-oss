<template>
    <BaseDrawer
        ...
        :title="title"
        :width="800"
        :confirm-btn-disabled="dataSeries?.disabled || Object.keys(errors).length > 0"
            ...
            <a-row class="form-row" :gutter="8">
                ...
                <a-col :span="8">
                    <FormInputSelect 
                        ...
                        :input-field-validation-schema="fieldValidationSchema.timeframeValue"
                        :select-field-validation-schema="fieldValidationSchema.timeframeUnit"
            ...
            <a-row :gutter="8">
                <a-col :span="4">
                    ...
                    <div style="position: relative;">
                        <div :class="dataSeries?.disabled ? 'cursor-not-allowed' : 'cursor-pointer'">
                            <div class="color-btn" :class="{ 'pointer-event-none': dataSeries?.disabled }"
                                :style="{ backgroundColor: enrichedColorHexCode }" @click="handleColorBtnClick"></div>
                        </div>
...
const fieldValidationSchema = {
    name: toTypedSchema(zod.string().nonempty(requiredErrorMessage).min(3).max(200).nullable()
        .refine(
            (value: string | null) => {
    ...
    valueName: toTypedSchema(zod.string().nonempty(requiredErrorMessage).nullable()),
    fieldName: toTypedSchema(zod.string().nullable().refine(value => {
        if (values.aggregationOperation === QueryAggregationOperator.COUNT) return true;
    ...
    timeframeValue: toTypedSchema(zod.coerce.number({ invalid_type_error: 'You must specify a number' }).nullable()),
    timeframeUnit: toTypedSchema(zod.string().nullable()),
    color: toTypedSchema(zod.string().nullable().refine((value) => ColorHelper.isValidHexCode(value as string))),
    colorOpacity: toTypedSchema(zod.coerce.number({ invalid_type_error: 'You must specify a number' }).min(1).max(100, 'Value must be between 1 and 100.').nullable())
}
...
const handleColorInput = () => {
    ...
    enrichedColorHexCode.value = ColorHelper.enrichHexCodeWithOpacity(values.color, values.colorOpacity)
}
...
const handleColorChange = (enrichedHexCodeWithOpacity: string) => {
    ...
    setFieldValue("colorOpacity", ColorHelper.getOpacity(enrichedHexCodeWithOpacity))
}
...
const _updateDataSeries = async (formValues: DataSeriesForm) => {
    const dataSeriesPatchRequests = SeriesHelper.toDataSeriesPatchRequest(initialFormValue, formValues);
    ...
    try {    
        await updateDataSeries(props.dataSeries!.reference, dataSeriesPatchRequests);
...
const _createDataSeries = async (formValues: DataSeriesForm) => {
    const dataSeriesPatchRequests = SeriesHelper.toDataSeriesCreationRequest(formValues);
...
const _shouldAggregationOperationFieldDisabled = (fieldName: string) => {
    ...
    if (selectedFieldOption?.type !== DataFieldType.NUMBER) {
        setFieldValue('aggregationOperation', QueryAggregationOperator.COUNT);
    }
    aggregationOperationFieldDisabled.value = selectedFieldOption?.type !== DataFieldType.NUMBER ? true : false
}
...
const _prepareValueNameFieldOptions = async (dataType: DataType) => {
    ...
    try {
        ...
        valueNameOptions.value = valueNames
            .map(valueName => ({
                label: valueName,
                value: valueName
            }));
